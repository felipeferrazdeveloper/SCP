        ...
        <div class="col-md-10">
            ...
            <div class="conteudo"> 
                ...
                <form action="ManterAlunoController?acao=prepararOperacao&operacao=Incluir" method="post">       
                    <input type="submit" name="btnIncluir" value="Incluir Aluno" class="btn btn-success" >
                </form>
                <br>
                ...
                <table class="table table-striped table-hover " id="table">
                   ...
                   <tbody>
                       <c:forEach items="${alunos}" var="aluno">
                           <tr>
                               <td><c:out value="${aluno.getIdAluno()}"/></td>
                               <td><c:out value="${aluno.getMatricula()}"/></td>
                               <td><c:out value="${aluno.getNome()}"/></td>
                               <td><c:out value="${aluno.getCpf()}"/></td>
                               <td><c:out value="${aluno.getCurso().getNome()}"/></td>
                               <td><a href="ManterAlunoController?acao=prepararOperacao&operacao=Editar&idAluno=<c:out value="${aluno.getIdAluno()}"/>" class="btn btn-warning btn-xs"> Editar</a></td>
                        <td><a href="ManterAlunoController?acao=prepararOperacao&operacao=Excluir&idAluno=<c:out value="${aluno.getIdAluno()}"/>" class="btn btn-danger btn-xs">Excluir</a></td> 
                           </tr>
                        </c:forEach>
                   </tbody> 
                </table> 
